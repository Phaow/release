...
ACCOUNT_B_ROLE_ARN=$(aws iam create-role \
  --role-name "${CLUSTER_NAME}-cross-account-aws-efs-csi-operator" \
...
oc create -n ${EFS_CSI_DRIVER_OPERATOR_INSTALLED_NAMESPACE} rolebinding efs-controller-access-secrets-rolebinding --role=efs-controller-access-secrets --serviceaccount=openshift-cluster-csi-drivers:aws-efs-csi-driver-controller-sa
